...
import kotlinx.android.synthetic.main.body_add_card.*
import me.iwf.photopicker.PhotoPicker
import me.iwf.photopicker.PhotoPreview


class AddCardActivity : CommonActivity(), ImageViewWithDelete.OnImgDeleteListener, OnItemClickListener {
    ...
    override fun getBodyLayoutResId(): Int {
        return R.layout.body_add_card
    }

    override fun initBodyView(view: View) {
        val recyclerView = findViewById<RecyclerView>(R.id.rv_select_img)
        ...
        bt_save_card.setOnClickListener({
            val cardType = intent.getIntExtra("cardType", 0)
            val type = et_add_card_type.text.toString()
            val cardName = et_add_card_name.text.toString()
            val cardDes = et_add_card_content.text.toString()

            if(StringUtils.isEmpty(cardName) || StringUtils.isEmpty(cardDes) || StringUtils.isEmpty(type)) return@setOnClickListener
            ...
            viewModel.insertCard(arrayListOf(entity))
        })
    }
